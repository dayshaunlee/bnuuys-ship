...
Entity createInvader(RenderSystem* renderer, vec2 position) {
    ...
    registry.renderRequests.insert(entity,
                                   {TEXTURE_ASSET_ID::INVADER, EFFECT_ASSET_ID::TEXTURED, GEOMETRY_BUFFER_ID::SPRITE});
...
Entity createTower(RenderSystem* renderer, vec2 position) {
    ...
    registry.renderRequests.insert(entity,
                                   {TEXTURE_ASSET_ID::TOWER, EFFECT_ASSET_ID::TEXTURED, GEOMETRY_BUFFER_ID::SPRITE});
...
Entity createLine(vec2 position, vec2 scale) {
    ...
    // Store a reference to the potentially re-used mesh object (the value is stored in the resource cache)
    registry.renderRequests.insert(
        entity, {// usage TEXTURE_COUNT when no texture is needed, i.e., an .obj or other vertices are used instead
                 TEXTURE_ASSET_ID::TEXTURE_COUNT, EFFECT_ASSET_ID::EGG, GEOMETRY_BUFFER_ID::DEBUG_LINE});
...
Entity createChicken(RenderSystem* renderer, vec2 pos) {
    ...
    registry.renderRequests.insert(
        entity, {// usage TEXTURE_COUNT when no texture is needed, i.e., an .obj or other vertices are used instead
                 TEXTURE_ASSET_ID::TEXTURE_COUNT, EFFECT_ASSET_ID::CHICKEN, GEOMETRY_BUFFER_ID::CHICKEN});
